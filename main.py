...
async def db_init():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(SQL_COUNTERS)
        for c in CATEGORIES:
            await db.execute(
                "INSERT OR IGNORE INTO counters (category, seekers_count, offers_count) VALUES (?,0,0)",
                (c,)
            )
        await db.commit()
...
def normalize_category(text: str) -> str:
    ...
    words = set(t.split())
    for c in CATEGORIES:
        sc = len(words & set(c.lower().split()))
        if sc > best_score:
...
async def seeker_finish(m: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO seekers (tg_id, full_name, category, region, experience, salary, contact, contact_visible)
               VALUES (?,?,?,?,?,?,?,?)""",
    ...
    if ADMIN_ID:
        try:
            await bot.send_message(ADMIN_ID, f"✅ Yangi ish qidiruvchi: {category}\n👥 Holat — ishchi: {s_count} | taklif: {o_count}")
        except Exception:
            pass
...
async def emp_finish(m: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """INSERT INTO offers (tg_id, full_name, category, region, headcount, salary, contact, contact_visible)
               VALUES (?,?,?,?,?,?,?,?)""",
    ...
    if ADMIN_ID:
        try:
            await bot.send_message(ADMIN_ID, f"✅ Yangi ish taklifi: {category}\n👥 Holat — ishchi: {s_count} | taklif: {o_count}")
        except Exception:
            pass
...
# -------------------- Runner --------------------
async def main():
    dp.include_router(router)     # router бир марта
    await db_init()               # база тайёр
